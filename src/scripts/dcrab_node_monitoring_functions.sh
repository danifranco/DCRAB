...
dcrab_node_monitor_init_variables () {
    ...
    if [ $DCRAB_INTERNAL_MODE -eq 0 ]; then
        ...
        # Discard odd values
        if [ $(echo "${DCRAB_IB_FIRST_XMIT_DATA_VALUE: -1} % 2" | bc) -eq 1 ]; then
            DCRAB_IB_FIRST_XMIT_DATA_VALUE=$((DCRAB_IB_FIRST_XMIT_DATA_VALUE + 1))
        else
            DCRAB_IB_XMIT_DATA_VALUE=$((DCRAB_IB_XMIT_DATA_VALUE + 1))
        fi
        if [ $(echo "${DCRAB_IB_FIRST_RCV_DATA_VALUE: -1} % 2" | bc) -eq 1 ]; then
            DCRAB_IB_FIRST_RCV_DATA_VALUE=$((DCRAB_IB_FIRST_RCV_DATA_VALUE + 1))
        else
            DCRAB_IB_RCV_DATA_VALUE=$((DCRAB_IB_RCV_DATA_VALUE + 1))
        fi
...
dcrab_determine_main_process () {
    ...
    cont=0
    # Initialize DCRAB_JOB_PROCESSES_FILE data file with the first processes
    for line in $(ps axo euid,sess,pid,pcpu,comm,command | awk '{if ($1 == '"$DCRAB_USER_ID"'){print}}' | awk '{if ($2 == '"$DCRAB_MAIN_SESSION"'){print}}' | awk '{if ($2 != '"$DCRAB_DCRAB_SESSION"'){print}}'); do
